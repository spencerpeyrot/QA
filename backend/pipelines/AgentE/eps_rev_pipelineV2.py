...
Assess reports against source documents using TRUE/FALSE criteria. Be objective and thorough in your evaluation."""
    ...
    async def correct_document(self, ev_doc: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not e["completeness_criteria"]["paragraphs_all_supported"]:
            issues.append("Some paragraphs lack source citations.")
        if not e["completeness_criteria"]["predictive_section_present"]:
            issues.append("Missing or incomplete Predictive Estimates section.")
        if not e["quality_criteria"]["clear_structure"]:
            issues.append("Structure needs improvement for clarity.")
        if not e["quality_criteria"]["predictive_section_consistent"]:
            issues.append("Predictive estimates section is inconsistent with main analysis.")
        ...
        system_prompt = """You are an expert financial editor specializing in EPS/Revenue analysis reports. 
Your task is to correct issues in financial reports while preserving accurate information."""
...
INSTRUCTIONS:
1. Fix all identified issues while preserving accurate information
2. Ensure all numbers match source documents exactly
3. Add/fix citations to match source documents
4. Maintain clear bullet-point structure
5. Ensure Predictive Estimates section is present and consistent
6. Keep similar length and tone as original
7. Return ONLY the corrected report
...
CORRECTED REPORT:"""
    ...
    async def process_corrections(self, start: datetime, end: datetime) -> Dict[str, Any]:
...
async def main():
    ...
    corr_results = await ev.process_corrections(start, end)
