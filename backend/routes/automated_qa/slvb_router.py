...
router = APIRouter(prefix="/slvb", tags=["slvb"])
...
async def run_pipeline_tasks(start_date: datetime, end_date: datetime):
    ...
    """
    try:
        logger.info("Initializing SLVBTickerEvaluator")
        evaluator = SLVBTickerEvaluator(MONGO_URI)
...
async def run_pipeline():
    """Start the SLVB pipeline evaluation and correction process"""
    logger.info("Received request to run SLVB pipeline")
    ...
    try:
        ...
        # PRODUCTION: start_date = end_date - timedelta(days=30)
        start_date = end_date - timedelta(days=3)  # TESTING: Only look at past 3 days
...
async def get_pipeline_status():
    """Get the current status of the SLVB pipeline"""
